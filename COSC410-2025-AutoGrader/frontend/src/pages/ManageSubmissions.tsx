...
export default function ManageSubmissions(){
  ...
  useEffect(()=>{ fetchJson("/api/v1/files?category=SUBMISSION").then(setFiles).catch(()=>{}) ; fetchJson("/api/v1/submissions").then(setList).catch(()=>{}) }, [])

  async function create(){
    const res = await fetchJson("/api/v1/submissions", { method: "POST", body: JSON.stringify({ name, file_ids: selected }) })
    setList([...list, res])
  }
