class IsolateJob < ApplicationJob
  ...
  def initialize_workdir
    ...
    @cgroups = ""  # Disable cgroups for now since they're not working in container
    ...
    source_content = submission.source_code
    # Decode base64 if it looks like base64 (contains only valid base64 chars and is properly padded)
    if source_content =~ /^[A-Za-z0-9+\/]+={0,2}$/ && (source_content.length % 4 == 0)
      begin
        source_content = Base64Service.decode(source_content)
      rescue
        # If decoding fails, use original content
      end
    end
    File.open(source_file, "wb") { |f| f.write(source_content) } unless submission.is_project
  ...
  def extract_archive
    ...
    #{submission.enable_per_process_and_thread_time_limit ? (cgroups.present? ? "--no-cg-timing" : "") : ""} \
