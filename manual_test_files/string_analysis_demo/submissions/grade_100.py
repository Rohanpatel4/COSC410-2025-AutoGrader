...
def longest_word(text: str) -> str:
    words = _words(text)
    return max(words, key=lambda w: (len(w), -ord(w[0]) if w else 0))


def shortest_word(text: str) -> str:
    words = _words(text)
    return min(words, key=lambda w: (len(w), words.index(w)))
...
def palindrome_words(text: str):
    pals = {w for w in _words(text) if len(w) > 1 and w == w[::-1]}
    return sorted(pals)


def reverse_sentences(text: str) -> str:
    sentences = [s.strip() for s in re.split(r"[.!?]", text) if s.strip()]
    reversed_order = list(reversed(sentences))
